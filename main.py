...
async def on_message(message):
  ...
  if message.content.lower().startswith("log"):
    if (db['scores'] != None):
      ...
      description = " "
      for score in end:
        description += "`" + score[1][10:19] + " ` :  `  " +  score[0] + " ` : ` " + score [2] + " ` : ` Score:" + str(score[3]) + " `\n\n"
  ...
  if message.content.lower().startswith("highscores"):
    ...
    for x in range (5):
      if (len(sorted_list) > x):
        description += "```" + str(x+1) + ". " + sorted_list[x][0] + "        Score: " + str(sorted_list[x][1]) + "``` \n"
